...
Describe "ConfigurationName" {
    $script:configFile = (join-path $PSScriptRoot 'machineConfig.csv')
    AfterAll {
            Remove-Item $script:configFile -ErrorAction SilentlyContinue -Force
    }
    Context "Default configuration name" {
        ...
        $configContent | Out-File -FilePath $script:configFile -Force
